// pages/blog/BlogPostDetail.tsx - Cleaned
...
const BlogPostDetail: React.FC = () => {
  ...
  const renderTableOfContents = () => {
    if (!post.enable_toc || !post.table_of_contents || post.table_of_contents.length === 0) return null;
    ...
    return (
      <div className="mb-8 border w-full max-w-[450px] border-gray-300 rounded-lg bg-white shadow-sm overflow-hidden">
        ...
        >
          ...
          {expandedTOC ? <ChevronUp className="w-6 h-6 text-gray-600" /> : <ChevronDown className="w-6 h-6 text-gray-600" />}
        </button>
  ...
  const renderContent = () => {
    if (!post.subheadings || post.subheadings.length === 0) {
      return <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: post.content }} />;
    }
      ...
      <div className="space-y-12">
        {post.subheadings.map((subheading: any, index: number) => (
          <div key={index} id={slugify(subheading.title)} className="scroll-mt-24">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">{subheading.title}</h2>
            <div
            ...
            {index === 0 && post.image_1 && (
              <div className="my-8">
                <img src={post.image_1} alt={`${subheading.title} illustration`} className="w-full h-auto rounded-lg shadow-md" />
              </div>
            )}
            {index === 2 && post.image_2 && (
              <div className="my-8">
                <img src={post.image_2} alt={`${subheading.title} illustration`} className="w-full h-auto rounded-lg shadow-md" />
              </div>
            )}
          </div>
        ))}
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-blue-600 mb-6 leading-tight">{post.title}</h1>
        ...
        {post.featured_image && (
          <div className="mb-8">
            <img src={post.featured_image} alt={post.title} className="w-full h-auto rounded-lg shadow-lg" />
          </div>
        )}
