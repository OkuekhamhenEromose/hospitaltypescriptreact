...
const LabScientistDashboard: React.FC = () => {
  ...
  const pName = (r: TestRequest): string => r.appointment?.name||"Unknown Patient";
  const pAge = (r: TestRequest): string => String(r.appointment?.age||"N/A");
  const pSex = (r: TestRequest): string => {
  ...
  async function submitResult(e: React.FormEvent): Promise<void> {
  ...
  function markInProgress(id: number): void {
    setRequests(p=>p.map(r=>r.id===id?{...r,status:"IN_PROGRESS"}:r));
  }
  
  function markDone(id: number): void {
    setRequests(p=>p.map(r=>r.id===id?{...r,status:"DONE"}:r));
  }
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",WebkitFontSmoothing:"antialiased",minHeight:"100vh",background:C.slate,display:"flex"}}>
      ...
      <aside style={{width:sw,minHeight:"100vh",background:C.white,display:"flex",flexDirection:"column",borderRight:`1px solid ${C.soft}`,position:"sticky",top:0,height:"100vh",overflow:"hidden",flexShrink:0,transition:"width 0.25s cubic-bezier(.4,0,.2,1)",boxShadow:"2px 0 10px rgba(79,70,229,0.06)"}}>
        <div style={{padding:"18px 14px 14px",borderBottom:`1px solid ${C.soft}`,display:"flex",alignItems:"center",gap:10,minHeight:62}}>
          <div style={{width:30,height:30,borderRadius:8,background:`linear-gradient(135deg,${C.indigo},#6366f1)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <UniversalIcon name="Beaker" size={16} style={{color:C.white}} />
          </div>
        ...
        <nav style={{flex:1,padding:"10px 8px",display:"flex",flexDirection:"column",gap:3}}>
          {navItems.map(n=>{
            const act=tab===n.id;
            return(
              <button key={n.id} className="nh" onClick={()=>setTab(n.id as typeof tab)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:collapsed?"10px":"10px 12px",borderRadius:10,border:"none",cursor:"pointer",background:act?`${C.indigo}12`:"transparent",justifyContent:collapsed?"center":"flex-start",transition:"all 0.15s"}}>
                <UniversalIcon name={n.icon} size={17} style={{color:act?C.indigo:C.muted,flexShrink:0}} />
        ...
        <div style={{borderTop:`1px solid ${C.soft}`,padding:"10px 8px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:10,background:C.slate}}>
            <div style={{width:30,height:30,borderRadius:"50%",background:`linear-gradient(135deg,${C.indigo},#6366f1)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,overflow:"hidden"}}>
              {imgUrl(user?.profile as unknown as Record<string, unknown>)?<img src={imgUrl(user?.profile as unknown as Record<string, unknown>)!} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<span style={{color:C.white,fontSize:12,fontWeight:600}}>{user?.profile?.fullname?.charAt(0)||"L"}</span>}
            </div>
        ...
        <header style={{background:C.white,borderBottom:`1px solid ${C.soft}`,padding:"0 28px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:30,boxShadow:"0 1px 8px rgba(79,70,229,0.06)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <button onClick={load} className="xh" style={{width:37,height:37,borderRadius:10,border:`1.5px solid ${C.soft}`,background:C.white,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}>
              <UniversalIcon name="RefreshCw" size={15} style={{color:C.muted}} />
            </button>
          ...
          <div style={{background:`linear-gradient(135deg,${C.indigo}08,${C.indigo}14)`,border:`1px solid ${C.soft}`,borderRadius:14,padding:"14px 20px",marginBottom:22,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <div style={{display:"flex",gap:28,alignItems:"center"}}>
              ...
              <button onClick={load} className="xh" style={{padding:"7px 14px",borderRadius:8,border:`1.5px solid ${C.soft}`,background:C.white,cursor:"pointer",fontSize:12.5,fontWeight:600,color:C.muted,display:"flex",alignItems:"center",gap:6,transition:"all 0.15s"}}>
                <UniversalIcon name="RefreshCw" size={13} />Refresh
              </button>
            </div>
          </div>
          ...
          <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.soft}`,overflow:"hidden",boxShadow:"0 1px 6px rgba(79,70,229,0.05)"}}>
            ...
            {filtered.length===0?(
              <div style={{padding:"60px 24px",textAlign:"center"}}>
                <UniversalIcon name="Beaker" size={38} style={{color:"#dce7f5",margin:"0 auto 10px"}} />
                <div style={{fontSize:15,fontWeight:600,color:C.muted}}>No test requests found</div>
                  ...
                  <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                          {testTags.length>0?testTags.map((t,ti)=>(
                            <span key={ti} style={{fontSize:11.5,fontWeight:600,padding:"3px 10px",borderRadius:20,background:`${C.indigo}10`,color:C.indigo,border:`1px solid ${C.indigo}25`,display:"flex",alignItems:"center",gap:4}}>
                              <UniversalIcon name="Beaker" size={10} />{t}
                            </span>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:7,flexShrink:0}}>
                      {(r.status==="PENDING"||r.status==="IN_PROGRESS")&&<>
                        {r.status==="PENDING"&&<button className="byh" onClick={()=>markInProgress(r.id)} style={{padding:"7px 13px",background:C.amber,color:C.white,borderRadius:8,border:"none",cursor:"pointer",fontSize:12.5,fontWeight:600,display:"flex",alignItems:"center",gap:5,transition:"all 0.15s"}}>
                          <UniversalIcon name="Clock" size={13} />Start Work
                        </button>}
                        <button className="bih" onClick={()=>{setSelected(r);setShowForm(true);}} style={{padding:"7px 13px",background:r.status==="PENDING"?C.blue2:C.indigo,color:C.white,borderRadius:8,border:"none",cursor:"pointer",fontSize:12.5,fontWeight:600,display:"flex",alignItems:"center",gap:5,transition:"all 0.15s",boxShadow:`0 2px 8px ${C.indigo}44`}}>
                          <UniversalIcon name="Beaker" size={13} />{r.status==="PENDING"?"Submit Results":"Update Results"}
                        </button>
                        {r.status==="IN_PROGRESS"&&<button className="bph" onClick={()=>markDone(r.id)} style={{padding:"7px 13px",background:C.green,color:C.white,borderRadius:8,border:"none",cursor:"pointer",fontSize:12.5,fontWeight:600,display:"flex",alignItems:"center",gap:5,transition:"all 0.15s"}}>
                          <UniversalIcon name="Check" size={13} />Mark Done
                        </button>}
                      </>}
                      {r.status==="DONE"&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"7px 13px",background:"#ecfdf5",borderRadius:8,border:"1px solid #bbf7d0",color:"#059669",fontSize:12.5,fontWeight:600}}>
                        <UniversalIcon name="Check" size={13} />Completed
                      </div>}
                    </div>
                  </div>
                </div>
