    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",WebkitFontSmoothing:"antialiased",minHeight:"100vh",background:C.slate,display:"flex"}}>
      ...
      <aside style={{width:sw,minHeight:"100vh",background:C.white,display:"flex",flexDirection:"column",borderRight:`1px solid ${C.soft}`,position:"sticky",top:0,height:"100vh",overflow:"hidden",flexShrink:0,transition:"width 0.25s cubic-bezier(.4,0,.2,1)",boxShadow:"2px 0 10px rgba(13,148,136,0.06)"}}>
        <div style={{padding:"18px 14px 14px",borderBottom:`1px solid ${C.soft}`,display:"flex",alignItems:"center",gap:10,minHeight:62}}>
          <div style={{width:30,height:30,borderRadius:8,background:`linear-gradient(135deg,${C.teal},#0f766e)`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <UniversalIcon name="Activity" size={16} style={{color:C.white}} />
          </div>
          ...
          <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.soft}`,overflow:"hidden",boxShadow:"0 1px 6px rgba(13,148,136,0.05)"}}>
            ...
            {filtered.length===0?(
              <div style={{padding:"60px 24px",textAlign:"center"}}>
                <UniversalIcon name="Activity" size={38} style={{color:"#dce7f5",margin:"0 auto 10px"}} />
                <div style={{fontSize:15,fontWeight:600,color:C.muted}}>No vital requests found</div>
                  ...
                  <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:7,flexShrink:0}}>
                      {(r.status==="PENDING"||r.status==="IN_PROGRESS")&&<>
                        {r.status==="PENDING"&&<button className="bth" onClick={()=>markInProgress(r.id)} style={{padding:"7px 13px",background:C.amber,color:C.white,borderRadius:8,border:"none",cursor:"pointer",fontSize:12.5,fontWeight:600,display:"flex",alignItems:"center",gap:5,transition:"all 0.15s"}}>
                          <UniversalIcon name="Clock" size={13} />Start
                        </button>}
                        <button className={r.status==="PENDING"?"bth":"boh"} onClick={()=>{setSelected(r);setShowForm(true);}} style={{padding:"7px 13px",background:r.status==="PENDING"?C.teal:C.orange,color:C.white,borderRadius:8,border:"none",cursor:"pointer",fontSize:12.5,fontWeight:600,display:"flex",alignItems:"center",gap:5,transition:"all 0.15s"}}>
                          <UniversalIcon name="Activity" size={13} />{r.status==="PENDING"?"Record Vitals":"Update Vitals"}
                        </button>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              <div>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12,padding:"8px 12px",background:"#fff1f2",borderRadius:8,border:"1px solid #fecdd3"}}>
                  <UniversalIcon name="Heart" size={14} style={{color:"#e11d48"}} />
                  <span style={{fontSize:12.5,fontWeight:700,color:C.text}}>Cardiovascular</span>
                </div>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12,padding:"8px 12px",background:"#fff7ed",borderRadius:8,border:"1px solid #fed7aa"}}>
                  <UniversalIcon name="Thermometer" size={14} style={{color:C.orange}} />
                  <span style={{fontSize:12.5,fontWeight:700,color:C.text}}>Respiratory & Temp</span>
                </div>
